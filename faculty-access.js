...
window.UCVM=(()=>{
 ...
 async function logs(container,{includeFaculty=false}={}){
  ...
  const $=id=>container.querySelector('#'+id),actionLabel=a=>({create:'Added',delete:'Deleted',update:'Changed',swap_faculty:'Faculty changed',replace_from_all_faculty_summaries:'Timetable synchronized',replace_and_sync_all_faculty_summaries:'Faculty/timetable synchronized',replace_away_from_campus_afc:'AFC imported',replace_workload_doe_breakdown_2026_27:'Workload DOE imported'})[a]||a||'Changed';
  function kindMatch(e,kind){if(!kind)return true;if(kind==='create'||kind==='delete')return e.action===kind;return(e.changes||[]).some(c=>kind==='time'?['date','start','end'].includes(c.field):kind==='name'?/name|topic/i.test(`${c.field} ${c.label}`):kind==='faculty'?/faculty|assignment|instructor/i.test(`${c.field} ${c.label}`):false)}
