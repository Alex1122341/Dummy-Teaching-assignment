...
(()=>{
 ...
 let me,groups=[],sessions=[],facultyRecord=null,weekStart=null,currentView='calendar';
 ...
 function show(view){currentView=view;const calendar=view==='calendar',list=view==='list',history=view==='history';$('calendar-view').hidden=!calendar;$('list-view').hidden=!list;$('history-panel').hidden=!history;$('sessions-panel').hidden=history;for(const [id,v] of [['calendar-tab','calendar'],['list-tab','list'],['history-tab','history']])$(id).classList.toggle('primary',view===v);if(history)UCVM.logs($('history-panel'));}
 ...
 $('signout').onclick=()=>auth.signOut();$('calendar-tab').onclick=()=>show('calendar');$('list-tab').onclick=()=>show('list');$('history-tab').onclick=()=>show('history');$('week-prev').onclick=()=>{weekStart=addDays(weekStart,-7);renderCalendar(mine())};$('week-next').onclick=()=>{weekStart=addDays(weekStart,7);renderCalendar(mine())};$('week-current').onclick=()=>{weekStart=null;renderCalendar(mine())};
 auth.onAuthStateChanged(async u=>{if(!u){location.replace('index.html');return}try{me=(await db.doc(`users/${u.uid}`).get()).data();if(!await UCVM.ready(u,me))return;me={...me,role:UCVM.role(me.role)};UCVM.watch(u,(await db.doc(`users/${u.uid}`).get()).data());$('identity').textContent=`${me.name||u.email} · ${UCVM.label(me.role)}`;$('management-link').hidden=!(UCVM.general(me)||me.role==='hicc');$('directory-link').hidden=!UCVM.admin(me);await loadGroups();facultyRecord=await resolveFaculty(u);setIdentityNote(u);$('session-note').textContent=['hicc','visc'].includes(me.role)?'Shows your own assigned sessions plus courses attached to your HICC group. Instructor replacement remains ADFA-only in Spark Basic Mode.':'Shows only sessions assigned to the Faculty Directory person verified by your signed-in email.';$('content').hidden=false;show(currentView);db.collection('sessions').onSnapshot(s=>{sessions=s.docs.map(d=>({id:d.id,...d.data()}));render()},err=>$('status').textContent=err.message)}catch(err){$('status').textContent=err.message}});
})();
